...
| 用户工作台（统计/趋势/积分） | 在线剪辑器（多轨道时间轴） |
|:---:|:---:|
| ![用户工作台](docs/screenshots/workbench.png) | ![在线剪辑器](docs/screenshots/editor.png) |

| 后台管理仪表盘（趋势/排行/成功率） | ComfyUI 工作流库 / 媒体资源管理 |
|:---:|:---:|
| ![后台仪表盘](docs/screenshots/admin-dashboard.png) | ![ComfyUI 工作流](docs/screenshots/comfyui-workflows.png) |

<details>
<summary>更多截图：媒体资源管理（视频缩略图）</summary>

![媒体资源管理](docs/screenshots/admin-media.png)
...
- ✅ **后台管理**：
  - 用户/项目/任务队列（分页筛选/批量操作/实时进度）、模型配置（含出站代理，支持 http/socks5）、提示词模板、计价规则、积分管理（充值/流水）
  - 仪表盘：今日新增/活跃、任务成功率、积分消耗、模型使用 Top6、近 7 日趋势图、最近失败任务
  - **统一媒体资源管理**：生成任务输出/素材库/项目资产/画布节点媒体五来源合并（搜索/禁用 403/删除/重命名，视频列表带缩略图）
