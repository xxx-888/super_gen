...
| 变量 | 默认 | 说明 |
|---|---|---|
| `FILE_SERVER_API_KEY` | 空 | 上传/列表/删除/统计的鉴权密钥。**留空 = fail-closed：所有写接口一律 403 拒绝**（仅直链下载与健康检查可用）。下载直链始终公开（渠道拉取不带鉴权） |
...
| `FILE_SERVER_ALLOWED_EXT` | 内置媒体白名单 | 允许的扩展名（逗号分隔小写）。默认图片/视频/音频共 21 种；设 `*` 不限制；自定义如 `mp4,mov,mp3,wav` |
| `FILE_SERVER_STRICT_SNIFF` | true | 魔数嗅探：扩展名合法但文件头不是对应媒体格式（伪装文件）一律拒绝；托管冷门格式可设 `false` |

建议用 nginx/caddy 反代到域名并配 HTTPS（渠道拉取 http 也可以，但浏览器混合内容限制下 https 更稳）。

## 安全机制

- **fail-closed 鉴权**：未配置密钥时写接口全部拒绝，杜绝"忘配 key = 公网裸奔"；密钥用 `hmac.compare_digest` 常量时间比较防时序攻击；鉴权失败写审计日志（IP/UA/动作）
- **上传三重校验**：鉴权 → 扩展白名单 → 文件头魔数嗅探（伪装成 `.png` 的脚本/木马在落盘前即被拒）→ 流式写入边收边查大小
- **路径防穿越**：删除与列表的路径先 `resolve()` 再校验必须落在存储目录内（`..`、符号链接、URL 编码绕过均拦截），外部文件不可能被误删/误列
- **审计日志**：上传/删除成功与所有鉴权失败均记录来源 IP，便于发现爆破与滥用
